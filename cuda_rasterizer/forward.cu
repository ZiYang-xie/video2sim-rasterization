...
__device__ float computeDynamicBaselineBlur(float focal_x, float focal_y, int screen_width, int screen_height) {
    // Example: scale based on focal length and screen size
    return 0.3f * (focal_x / screen_width + focal_y / screen_height) * 0.5f;
}


// Forward version of 2D covariance matrix computation
__device__ float3 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* cov3D, const float* viewmatrix, int screen_width, int screen_height)
{
  ...
  // Compute perspective scaling factors based on depth
  ...
  // const float splatSizeX = sqrt(cov[0][0]);
  // const float splatSizeY = sqrt(cov[1][1]);
  // const float max_portion = 0.15f;

  // // If the Gaussian is too large, discard it
  // if (splatSizeX > max_portion * screen_width || splatSizeY > max_portion * screen_height)
